...
def aug_mega_hardcore(prob=.5):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=45, prob=.2),
        OneOf([
            Distort1(prob=0.3),
            Distort2(prob=.1),
            IAAPiecewiseAffine(prob=0.3),
...
def aug_gray(prob=.5):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.4, rotate_limit=45, prob=.4),
        OneOf([
            Distort1(prob=0.3),
            Distort2(prob=.1),
            IAAPiecewiseAffine(prob=0.3),
...
def aug_victor(prob=0.9):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=.0625, scale_limit=0.4, rotate_limit=45, prob=.7),
        OneOf([
            Distort1(prob=.2),
            Distort2(prob=.2),
